    ...
    def get_matrix(self):
        try:
            ...
            for i in range(0, m * n, n):
                res.append([float(eval(ent.get())) for ent in self.entries_list[i:i + n]])
            return res
        except:
...
class Operations(ctk.CTkFrame):
    def __init__(self, master, linked_matrix):
        ...
        self.number.grid(row=4, column=0, padx=(210, 10))
        ...
        self.power.grid(row=5, column=0, padx=(210, 10))

    def transpose_btn_event(self):
        arr = self.linked_matrix.get_matrix()
        if isinstance(arr, list):
            matrix = transpose_matrix(arr)
...
class Slau(CTkXYFrame):  # решение СЛАУ
    ...
    def result_slau(self):
        ...
        try:
            A = []
            B = [float(i.get().replace(',', '.')) for i in self.entries_B]
            for value in self.entries_A:
                l = [float(i.get().replace(',', '.')) for i in value]
                A.append(l)
            if rang_check(A,B):
                self.output = ctk.CTkLabel(self.result_frame, text=f"СЛАУ несовместна", width=180)
                self.output.grid(row=0, column=0, padx=10, pady=10, sticky='news')
            elif matrix_rang(A)<np.array(A).shape[0]:
                self.output = ctk.CTkLabel(self.result_frame, text=f"СЛАУ имеет бесконечное множество решений", width=180)
                self.output.grid(row=0, column=0, padx=10, pady=10, sticky='news')
            elif determinant(A) == 0:
                self.output = ctk.CTkLabel(self.result_frame, text=f"Определитель СЛАУ равне 0", width=180)
                self.output.grid(row=0, column=0, padx=10, pady=10, sticky='news')
            else:
                result = solution_SLAU(A, B)
                count=0
                for i in result:
                    self.output = ctk.CTkLabel(self.result_frame, text=f"X{count+1} = {round(i,4)}", width=180)
                    self.output.grid(row=count, column=0, padx=10, pady=10, sticky='news')
                    count+=1
            """Здесь происходит основные задача. A - двухмерный массив,каждая строка содержит все значения при неизвестных.
                     В - одномерный массив хранит столбец свободных членов. """
        except ValueError:
            self.output = ctk.CTkLabel(self.result_frame, text=f"Вы ввели некорректные данные", width=180)
            self.output.grid(row=0, column=0, padx=10, pady=10, sticky='news')
